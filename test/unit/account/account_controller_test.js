...
describe('controllers', function(){
  ...
  describe('AccountCtrl', function(){
    var AccountCtrl, Account,$rootScope, $scope, $routeParams, $httpBackend, $location, MessageHandler, $q, $controller;
    ...
    it('refreshAccount', function() {
    	...
    	Account.get = function(uid) {
			var deferred = $q.defer();
			deferred.resolve({data:'account'+uid});
			return deferred.promise;
		}
    ...
    it('goToAccount', function() {
    	...
    	// when
    	$scope.goToAccount(uid);
    	$scope.$apply();
    	
    	// then
    	expect($scope.refreshAccount).toHaveBeenCalledWith(uid);
    	expect($location.path).toHaveBeenCalledWith('/account'+'/'+uid);
    });
    
    it('save : create', function() {
    	// given
    	$scope.account = {uid:'1', name:'account'};
