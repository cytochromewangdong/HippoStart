...
describe('services', function(){
  ...
  describe('Account', function(){
    ...
    it('create throws exception : id not defined', function() {
    	try{
    		Account.create({uid:null,name:'account'});
    		$httpBackend.flush();
    	} catch(errors) {
    ...
    it('create', function() {
    	...
    	Account.create({uid:'1',name:'account'}).then(function(response) {
        	expect(response.status).toBe(200);
    		expect(response.data).toBe("test");
    	});
    	$httpBackend.flush();
    ...
    it('update throws exception : id not defined', function() {
    	try{
    		Account.update({uid:null,name:'account'});
    		$httpBackend.flush();
    	} catch(errors) {
    		expect(errors[0]).toBe('account.id.not.defined');
    	}
    ...
    it('update', function() {
    	...
    	Account.update({uid:'1',name:'account'}).then(function(response) {
        	expect(response.status).toBe(200);
    		expect(response.data).toBe("test");
    	});
    	$httpBackend.flush();
