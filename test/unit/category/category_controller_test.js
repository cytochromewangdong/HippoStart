...
describe('controllers', function(){
  ...
  describe('CategoryCtrl', function(){
    ...
    it('refreshCategory', function() {
    	// given
    	Category.get = function(corpid, uid) {
			var deferred = $q.defer();
			deferred.resolve({data:'category'+corpid+uid});
			return deferred.promise;
		}
    	
    	// when
    	$scope.refreshCategory('1', '2');
    	$scope.$apply();
    	
    	// then
    	expect($scope.category).toBe('category'+'1'+'2');
    });
    ...
    it('goToCategory', function() {
    	...
    	// when
    	$scope.goToCategory(corpid, uid);
    	$scope.$apply();
    	
    	// then
    	expect($scope.refreshCategory).toHaveBeenCalledWith(corpid, uid);
    	expect($location.path).toHaveBeenCalledWith('/category'+'/'+corpid+'/'+uid);
    });
    
    it('save : create', function() {
    	// given
    	$scope.category = {corpid:'1', uid:'2', name:'category'};
